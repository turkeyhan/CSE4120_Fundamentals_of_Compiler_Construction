...
let rec step ir_map label_map reg_map mem pc cost =
  let instr = IRMap.find pc ir_map in
  let cost' = cost + (Cost.measure instr) in
  match exec_instr label_map instr reg_map mem pc with
  | Finished v ->
      printf "Result: %s, Cost: %s\n" (string_of_value v) (string_of_int cost')
  | Running (reg_map', mem', pc') ->
      step ir_map label_map reg_map' mem' pc' cost'
...
let run (ir: ir_code) (arg_strs: string list): unit =
  ...
  step ir_map label_map reg_map Memory.empty 0 0
