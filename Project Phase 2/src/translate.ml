...
let rec trans_exp exp sym_tab =
  ...
  | Arr (vname, e_idx) ->
    let (instlist_e, reg_e) = trans_exp e_idx sym_tab in
    if SymbolMap.mem vname sym_tab = true then
      let addr = SymbolMap.find vname sym_tab in
      ...
      (failwith("Not found: " ^ vname), reg)
  | Add (e1, e2) ->
    let (instlist_e1, reg_e1) = trans_exp e1 sym_tab in
    let (instlist_e2, reg_e2) = trans_exp e2 sym_tab in
    let reg = create_register_name() in
  ...
  | Neg e ->
    let (instlist_e, reg_e) = trans_exp e sym_tab in
    let reg = create_register_name() in
      ...
      | Assign (lv, e) ->
        ...
        | LVar v ->
          ...
          if SymbolMap.mem v sym_tab = true then
            ...
            (instlist_e @ [Store(Reg reg_e, reg)], sym_tab)
          else
            (instlist_e @ failwith("Not found: " ^ v), sym_tab)
        | LArr (v, ea) ->
          let (instlist_e, reg_e) = trans_exp e sym_tab in
          let (instlist_ea, reg_ea) = trans_exp ea sym_tab in
